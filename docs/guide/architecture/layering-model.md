...
```mermaid
graph TD
    CONFIG["configuration<br/>única capa que puede depender de todas"] --> CTRL["presentation<br/>(controller)"]
    CONFIG --> DOMAIN["domain<br/>(model + service)"]
    CONFIG --> DS["datasource<br/>(repository)"]
    CTRL --> DOMAIN
    DS --> DOMAIN
...
```mermaid
sequenceDiagram
    participant Ctrl as Controller (presentation.controller)
    participant Svc as Service (domain.service, interfaz)
    participant Dec as Decorator (domain.service.impl, opcional)
    participant Impl as Implementación real (domain.service.impl)
    participant Repo as Repository (datasource.repository)
    participant Model as Model/Entity (domain.model)

    Ctrl->>Svc: llama a la interfaz (inyectada por configuration)
    Svc->>Dec: si existe variante decorada (p. ej. caché)
    Dec->>Impl: delega en la implementación real
    Impl->>Repo: lee/escribe datos
    Repo->>Model: opera sobre entidades de dominio
    Model-->>Ctrl: resultado (entidad o colección) hasta el controller
```
